...
export class Pickup {
  constructor(scene, type, x, z) {
    ...
    this.mesh = new THREE.Mesh(_geo, new THREE.MeshStandardMaterial({
  ...
  get label()    { return PICKUP_DEFS[this.type].label; }
...
export function spawnPickups(scene, bounds, count) {
  ...
  while (typeList.length < count) typeList.push(types[Math.floor(Math.random() * types.length)]);

  for (const type of typeList) {
    const margin = 3;
    const x = bounds.minX + margin + Math.random() * (bounds.maxX - bounds.minX - margin * 2);
    const z = bounds.minZ + margin + Math.random() * (bounds.maxZ - bounds.minZ - margin * 2);
...
export function spawnPickupsAt(scene, types, cx, cz) {
  return types.map((type, i) => {
    ...
    return new Pickup(scene, type, cx + Math.cos(angle) * r, cz + Math.sin(angle) * r);
  });
}
