...
const DEFAULT_WEIGHTS = [
  [
    1.2724493602627593, 1.7710790909490002, 2.994319701314286,
    0.9272276482448202,
  ],
  [1.2666666666666666, 1, 1, 1.2627354285714287],
  [
    1.4317323634632861, 0.6017008706075702, 3.183766295714286,
    2.8427306359523814,
  ],
  [1, 1.3214285714285714, 1.1738095238095239, 1],
];
...
export class TacticSelector {
  ...
  train(chosenIdx, challengeScore) {
    ...
    this._w[this._state][chosenIdx] = Math.min(
      8.0,
      this._w[this._state][chosenIdx] + challengeScore * 0.5,
    );
    localStorage.setItem("vec_nn_q", JSON.stringify(this._w));
  }
  ...
  summary(waveCount) {
    ...
    const avg = TACTICS.map(
      (_, a) => this._w.reduce((s, row) => s + row[a], 0) / 4,
    );
